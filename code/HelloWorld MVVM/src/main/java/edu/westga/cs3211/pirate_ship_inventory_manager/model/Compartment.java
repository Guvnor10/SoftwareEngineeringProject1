 ...
 * Class
 * 
 * @author gn00021 Represents a storage compartment on the ship that can hold
 *         stock items. Each compartment has a capacity (in quantity units) and
 *         may only store stock that matches its special qualities
 *         (StockAttributes).
 * @version Fall 2025r
 */
public class Compartment {
	 ...
	 *
	 * @param capacity         the maximum quantity units this compartment can hold
	 *                         (must be > 0)
	 * @param specialQualities the maximum specialQualities this compartment can
	 *                         have must be > 2
	 */
	...
	/**
	 * Computes how much free capacity remains in this compartment. This is based on
	 * the sum of the quantities of all stock items stored here.
	 *
	...
	/**
	 * Determines whether this compartment can store the given stock item, based on
	 * both free space and special qualities.
	 *
	...
	public boolean removeStock(Stock target) {
		...
		for (int i = 0; i < this.stockItems.size(); i++) {
			...
			boolean sameExp = java.util.Objects.equals(current.getExpirationDate(), target.getExpirationDate());
	...
	public String toString() {
		return "Cap:" + this.capacity + ", Free:" + this.getFreeSpace() + ", Qualities:" + this.specialQualities;
	}
}
