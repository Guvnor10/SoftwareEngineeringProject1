...
/**
 * Splits a single CSV line into columns while honoring: Quoted fields Escaped
 * quotes inside quoted fields (two quotes become one) Commas as separators only
 * when outside quotes
 * 
 * @author Yeni
 * @version Fall 2025
...
public final class CsvLineSplitter {
	...
	public static List<String> split(String line) {
		...
		while (index < length) {
			...
			if (inQuotes) {
				...
				OutQuotesOutcome outcome = processCharOutOfQuotes(ch, current, columns, delimiter);
	...
	// Change signature to pass delimiter
	private static OutQuotesOutcome processCharOutOfQuotes(char ch, StringBuilder current, List<String> columns,
			char delimiter) {
		...
		boolean isSeparator = (ch == delimiter);
		if (isSeparator) {
			columns.add(current.toString());
			current.setLength(0);
	...
	/**
	 * Outcome for handling a character while in quoted mode. Encapsulates flags to
	 * satisfy Checkstyle's accessor rules.
	 */
	private static final class InQuotesOutcome {
		 ...
		 *
		 * @param consumedExtraChar {@code true} if the next character was consumed
		 *                          (escaped quote)
		 * @param toggleQuotedState {@code true} if the caller should toggle quoted
		 *                          state (closing quote)
		 */
	...
	/**
	 * Outcome for handling a character while outside quotes. Encapsulates flags to
	 * satisfy Checkstyle's accessor rules.
	 */
	 ...
	 * <li>Escaped quote ({@code \"\"}): append one quote and mark extra char as
	 * consumed</li>
	 ...
	 */
	private static InQuotesOutcome processCharInQuotes(String line, int length, int index, char ch,
